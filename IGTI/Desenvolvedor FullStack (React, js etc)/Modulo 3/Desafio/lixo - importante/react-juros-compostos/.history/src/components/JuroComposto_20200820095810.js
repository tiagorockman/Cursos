...
export default function JuroComposto({ montanteInicial, taxaJuros, meses }) {
  ...
  useEffect(() => {
    ...
    const chartLabels = [];
    const chartAccumulatedMoney = [];
    const chartTotalInterest = [];
    chartLine.forEach((chart) => {
      chartLabels.push(chart.mes);
      chartAccumulatedMoney.push(char.valorMensal);
      chartTotalInterest.push(char.valorMensalMaisJuros);
      console.log(chartLabels);
      console.log(chartAccumulatedMoney);
      console.log(chartTotalInterest);
    });
    chartLine.map((chart) => {
      setDataChartLine({
        labels: [0, chart.mes],
        datasets: [
          {
            ...
            data: [capitalInicial, chart.valorMensal],
            fill: false,
          ...
          },
          {
            ...
            data: [0, chart.valorMensalMaisJuros],
            fill: false,
