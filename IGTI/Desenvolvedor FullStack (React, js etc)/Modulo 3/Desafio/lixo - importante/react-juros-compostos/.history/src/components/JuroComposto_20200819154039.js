...
export default function JuroComposto({ montanteInicial, taxaJuros, meses }) {
  ...
  const [capitalInicial, setCapitialInicial] = useState(capitalInicial);
  const [jurosMensal, setJurosMensal] = useState(jurosMensal);
  const [periodoMeses, setPeriodoMeses] = useState(periodoMeses);
