...
export default function JuroComposto({ montanteInicial, taxaJuros, meses }) {
  ...
  useEffect(() => {
    ...
    const chartInitialCapital = [capitalInicial];
    ...
    chartLine.forEach((chart) => {
      ...
      chartInitialCapital.push(capitalInicial);
    });
  ...
  return (
    <div>
      ...
      >
        ...
        <Chart type="line" data={dataChartLine} style={{ marginTop: "20px", display: "flex", flexDirection: 'row', alignContent: 'center' }}/>
      </div>
    </div>
  );
}
